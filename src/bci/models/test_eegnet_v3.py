"""EEGNet improvement v3: 9 subjects, batch=16, + data augmentation."""
import mne
import moabb
import torch
...
class AugmentedEEGDataset(Dataset):
    """Applies random Gaussian noise + small time shift during training."""
    def __init__(self, X, y, augment=False, noise_std=0.05, max_shift=10):
        self.X = X
        self.y = y
...
print("\nTraining EEGNet WITH AUGMENTATION (9 subjects, batch=16, max 150 epochs)...")
...
print("\n--- EEGNet v3 Results (9 subjects, augmented) ---")
for k, v in results.items():
    print(f"  {k}: {v}")

print("\n✅ EEGNet v3 (augmented) works end-to-end!")
