"""Train ShallowConvNet on 8 pooled subjects — same setup as EEGNet for fair comparison."""
import mne
import moabb
import torch
...
# Normalize per-channel — ShallowConvNet is sensitive to input scale
...
print(f"\nBuilding ShallowConvNet: {n_channels} channels, {n_classes} classes, {n_times} timepoints")
model = build_shallow_convnet(n_channels, n_classes, n_times)
lightning_model = LightningWrapper(model, lr=1e-4)

train_loader = DataLoader(TensorDataset(X_train, y_train), batch_size=16, shuffle=True)

print("\nTraining ShallowConvNet for 50 epochs (lr=1e-4, grad clip)...")
...
print("\n--- ShallowConvNet Results (8 subjects pooled) ---")
for k, v in results.items():
    print(f"  {k}: {v}")

print("\n✅ ShallowConvNet works end-to-end!")
